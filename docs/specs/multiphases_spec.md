...
**Текущее состояние:** Версия 2.1 с частичной поддержкой многофазных расчётов, где система использует температурный диапазон, указанный пользователем, для фильтрации записей из базы данных.

**Проблема:** Использование только пользовательского температурного диапазона приводит к потере критически важных данных (H₂₉₈, S₂₉₈), которые содержатся в записях, покрывающих стандартную температуру 298K, но не попадающих в запрашиваемый диапазон.
...
```sql
SELECT formula, phase, tmin, tmax, h298, s298, f1, f2, f3, f4, f5, f6, tmelt, tboil
FROM thermo_data
...
**Результат:**
```
| formula | phase | tmin   | tmax | h298     | s298   | f1    | f2   | f3    | tmelt | tboil |
| ------- | ----- | ------ | ---- | -------- | ------ | ----- | ---- | ----- | ----- | ----- |
...
1. **H₂₉₈ и S₂₉₈ заполнены только в первой записи каждой фазы**, покрывающей стандартную температуру
   ...
   - Для жидкой фазы (l): в записи, начинающейся с T_melt

2. **Температуры переходов (tmelt, tboil) одинаковы** во всех записях
   - tmelt = 1650K (температура плавления)
   - tboil = 3687K (температура кипения)
...
**Критерий приёмки:**
- Независимо от запрашиваемого диапазона, система находит все записи
- Расчёт всегда выполняется от 298K (или min_temp) до max_temp
- Пользовательский диапазон показывается только для справки
