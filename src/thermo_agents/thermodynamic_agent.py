...
LLM-компонент гибридной архитектуры v2.0 для извлечения термодинамических параметров
из запросов на естественном языке. Использует PydanticAI для структурированного вывода
и AgentStorage для асинхронной коммуникации.

Ключевые компоненты:
- ThermoAgentConfig: Конфигурация агента (API ключи, модель, storage)
- ThermodynamicAgent: Основной класс агента с event loop
- PydanticAI Agent: Внутренний агент для извлечения параметров

Основные методы:
- start(): Запустить агента в режиме прослушивания сообщений
- stop(): Остановить агента
- extract_parameters(user_query): Извлечь параметры из текстового запроса
- process_single_query(user_query): Обработать одиночный запрос
- _process_message(message): Обработать входящее сообщение из storage
- _initialize_agent(): Создать PydanticAI агент с инструментами

Инструменты PydanticAI:
- save_to_storage(key, value, ttl): Сохранить данные в хранилище
- load_from_storage(key): Загрузить данные из хранилища

Процесс обработки:
1. Получает сообщение "extract_parameters" из хранилища
2. Извлекает user_query из payload
3. Использует PydanticAI для извлечения структурированных параметров
4. Сохраняет результат в хранилище с TTL
5. Отправляет ответное сообщение "response" с результатом

Извлекаемые параметры:
- intent: Тип запроса (reaction/compound)
- compounds: Список химических соединений
- temperature_k: Температура в Кельвинах
- temperature_range_k: Диапазон температур
- phases: Фазовые состояния
- properties: Запрашиваемые свойства
- reaction_equation: Уравнение реакции (для реакций)
- sql_query_hint: Подсказка для SQL запроса

Обработка ошибок:
- Определение типов ошибок (auth, rate limit, network, timeout)
- Логирование ошибок с детализацией
- Отправка error-сообщений через storage
- Retry механизм с увеличением таймаута

Интеграция:
- Работает с AgentStorage для коммуникации
- Использует SessionLogger для логирования
- Интегрируется с ThermoOrchestrator v2.0
- Поддерживает ExtractedReactionParameters модель вывода

Фабричные функции:
- create_thermo_agent(): Создать настроенного агента
- run_thermo_agent_standalone(): Запустить в standalone режиме

Используется в:
- ThermoOrchestrator для извлечения параметров
- Standalone режиме для тестирования
- Message-passing архитектуре v2.0
"""
