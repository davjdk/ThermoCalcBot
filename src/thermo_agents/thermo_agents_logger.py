...
Централизованная система сессионного логирования для гибридной архитектуры v2.0.
Создает отдельные лог-файлы для каждой сессии с поддержкой Unicode,
форматированием таблиц через tabulate и детальным трейсингом операций.

Ключевые компоненты:
- SimpleOperationTimer: Контекст менеджер для измерения времени операций
- SessionLogger: Основной класс сессионного логгера
- create_session_logger(): Фабричная функция для создания логгеров

Основные методы SessionLogger:
- log_info()/log_error(): Базовое логирование сообщений
- create_operation_context(): Создание контекста для операций с таймером
- format_table(): Форматирование данных в таблицы через tabulate
- log_compound_data_table(): Логирование данных по соединениям
- log_detailed_compound_records(): Детальное логирование записей
- log_search_metadata(): Логирование метаданных поиска
- log_stage_header(): Логирование заголовков стадий фильтрации
- log_stage_statistics(): Детальная статистика стадий
- log_filter_stage_table(): Таблицы результатов фильтрации
- log_llm_interaction(): Логирование взаимодействия с LLM

Особенности реализации:
- Unicode очистка сообщений для кроссплатформенной совместимости
- Автоматическое создание директории логов (logs/sessions/)
- Генерация уникальных ID сессий на основе timestamp
- Двойной вывод: в консоль и в файл с UTF-8 кодировкой
- Fallback механизмы для форматирования таблиц
- Интеграция с OperationType для типизации операций

Форматирование таблиц:
- Использует tabulate с grid форматированием
- Поддержка ограничений на количество строк (max_rows)
- Очистка Unicode символов в заголовках и ячейках
- Fallback на простой формат при ошибках tabulate

Логирование операций:
- Автоматическое измерение времени выполнения
- Уникальные ID операций для трейсинга
- Корреляционные ID для связи операций
- Статус выполнения (started/completed/failed)
- Интеграция с AgentStorage для снапшотов

Специализированное логирование:
- Стадии фильтрации с визуальными разделителями
- Температурные диапазоны и распределение фаз
- Статистика надежности и покрытия данных
- Детальные метрики для каждого типа стадии
- LLM взаимодействия с временем выполнения

Безопасность и совместимость:
- Санитизация Unicode символов для Windows
- Обработка ошибок кодирования
- Graceful degradation при проблемах с форматированием
- Поддержка различных типов данных (int, float, dict, list)

Интеграция:
- Используется ThermodynamicAgent для логирования LLM операций
- Интегрируется с FilterPipeline для логирования стадий
- Поддерживается CompoundSearcher для логирования поиска
- Совместим с AgentStorage для создания снапшотов

Используется в:
- ThermodynamicAgent для логирования извлечения параметров
- FilterPipeline для логирования стадий фильтрации
- CompoundSearcher для логирования результатов поиска
- Orchestrator для трейсинга выполнения запросов
"""
