...
Профессиональный форматтер таблиц для термодинамических данных с использованием библиотеки tabulate.
Обеспечивает структурированное отображение результатов поиска с поддержкой различных форматов
и локализацией для русскоязычного интерфейса.

Основной класс:
- TableFormatter: Форматирование таблиц через tabulate

Структура таблицы (строгий порядок колонок):
1. **Формула**: Химическая формула соединения
2. **Название**: Наименование соединения
3. **Фаза**: Агрегатное состояние (s/l/g/aq)
4. **T_диапазон (K)**: Температурный диапазон применимости
5. **Tmelt (K)**: Температура плавления
6. **Tboil (K)**: Температура кипения
7. **H298 (кДж/моль)**: Стандартная энтальпия образования
8. **S298 (Дж/моль·K)**: Стандартная энтропия
9. **Cp_коэффициенты (f1-f6)**: Коэффициенты теплоемкости
10. **Надёжность (класс)**: Класс надежности данных

Основные методы:
- format_summary_table(): Сводная таблица термодинамических свойств
- format_detailed_table(): Детальная таблица с множественными записями
- format_compact_table(): Компактная таблица для отчетов
- format_comparison_table(): Сравнительная таблица соединений

Форматирование данных:
- _format_formula(): Форматирование химических формул
- _format_name(): Обработка названий соединений
- _format_phase(): Отображение фазовых состояний
- _format_temperature_range(): Диапазоны температур
- _format_melting_point()/boiling_point(): Фазовые переходы
- _format_h298()/_s298(): Термодинамические свойства
- _format_cp_coefficients(): Коэффициенты теплоемкости
- _format_reliability(): Классы надежности
...
- Форматирование температурных диапазонов

Поддержка различных форматов:
- Grid: Основной формат с рамками
- Simple: Упрощенный формат
- Pipe: Вертикальные разделители
- Fancy_grid: Декоративный формат

Обработка граничных случаев:
- Отсутствие данных для отображения
- Пустые результаты поиска
- Неопределенные значения свойств
- Множественные фазы одного соединения

Интеграция с результатами поиска:
- Поддержка CompoundSearchResult
- Совместимость с MultiPhaseSearchResult
- Обработка DatabaseRecord
- Адаптация под разные версии моделей

Оптимизации:
- Минимизация строковых операций
- Кэширование отформатированных значений
- Эффективная обработка больших таблиц
- Управление памятью при форматировании

Кастомизация:
- Настраиваемые заголовки таблиц
...
Интеграция:
- Используется ThermoOrchestrator для финального вывода
- Интегрируется с ReactionAggregator результатами
- Поддерживает SessionLogger для логирования
- Совместим с остальными компонентами системы

Используется в:
- ThermoOrchestrator для финального форматирования
- CLI приложении для вывода результатов
- API endpoints для табличных данных
