...
Централизованное хранилище для обмена данными между агентами в гибридной архитектуре v2.0.
Использует SimpleAgentStorage как backend для быстрого Key-Value хранения с поддержкой TTL.
Обеспечивает совместимость с существующим API через адаптерный паттерн.

Ключевые компоненты:
- AgentMessage: Модель сообщения между агентами с корреляционными ID
- StorageEntry: Устаревшая модель записи для совместимости
- AgentStorage: Основной класс хранилища с делегированием в SimpleAgentStorage

Методы управления данными:
- set(key, value, ttl_seconds): Сохранить данные с опциональным TTL
- get(key, default): Получить данные из хранилища
- delete(key): Удалить данные
- exists(key): Проверить существование ключа
- clear(): Очистить хранилище

Методы обмена сообщениями:
- send_message(): Отправить сообщение между агентами
- send_message_with_ack(): Отправить с подтверждением получения
- acknowledge_message(): Подтвердить получение сообщения
- receive_messages(): Получить сообщения для агента
- diagnose_message_flow(): Диагностировать поток сообщений

Методы управления сессиями:
- start_session(): Начать сессию для агента
- get_session(): Получить данные сессии
- update_session(): Обновить сессию
- end_session(): Завершить сессию

Вспомогательные методы:
- get_stats(): Получить статистику хранилища
- cleanup_expired(): Очистить просроченные записи
- get_storage_snapshot(): Получить снимок состояния
- to_dict()/from_dict(): Сериализация/десериализация

Singleton pattern:
- get_storage(): Получить глобальное хранилище
- reset_storage(): Сбросить глобальное хранилище

Используется в:
- Message-passing коммуникации между агентами
- Хранении промежуточных результатов обработки
- Отслеживании сессий и корреляционных ID
- Диагностике коммуникации между компонентами
"""
