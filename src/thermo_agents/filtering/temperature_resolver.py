...
Высокопроизводительный резолвер для работы с температурными диапазонами в термодинамической
базе данных. Реализует оптимизированные алгоритмы слияния интервалов, кэширование
и детерминированную логику для максимальной производительности.

Ключевые компоненты:
- TemperatureInterval: Модель температурного интервала с метаданными
- TemperatureResolver: Основной класс резолвера с кэшированием
- LRU cache и TTL оптимизации для часто используемых расчетов

TemperatureInterval:
- Представляет температурный диапазон [tmin, tmax]
- Содержит метаданные: source_record_id, phase, reliability_class
- Методы: contains(), overlaps_with(), intersection(), union()
- Валидация диапазонов и проверка корректности

Основные методы TemperatureResolver:
- check_coverage(): Высокопроизводительная проверка покрытия с кэшированием
- get_covered_ranges(): Получение объединенных температурных диапазонов
- get_gaps(): Поиск пробелов в покрытии температурного диапазона
- calculate_coverage_percentage(): Расчет процента покрытия
- get_temperature_statistics(): Статистика температурных данных
- clear_cache(): Очистка кэша результатов

Оптимизации производительности:
- LRU кэш с ограниченным размером (1000 записей)
- TTL для кэшированных результатов (15 минут)
- Оптимизированные алгоритмы слияния интервалов O(n log n)
- Пакетная обработка для больших наборов данных
- Предвычисленные хэши для ключей кэша
- Быстрые алгоритмы проверки пересечений

Алгоритмы работы:
1. Извлечение интервалов из DatabaseRecord
2. Сортировка интервалов по начальной температуре
3. Слияние пересекающихся/смежных интервалов
4. Расчет покрытия относительно целевого диапазона
5. Определение статуса (full/partial/none)

Кэширование:
- Coverage cache для результатов проверки покрытия
- Interval cache для объединенных интервалов
- TTL механизм для устаревания данных
- Автоматическая очистка при превышении лимита
- Метрики cache hit rate для мониторинга

Статус покрытия:
- 'full': >= 95% целевого диапазона покрыто
- 'partial': 0-95% покрыто
- 'none': нет покрытия

Анализ пробелов:
- Выявление незакрытых температурных диапазонов
- Определение размеров и расположения пробелов
- Помощь в идентификации проблемных областей

Метрики производительности:
- Cache hit rate и эффективность
- Время выполнения операций
- Размер кэша и использование памяти
- Статистика по обработанным интервалам

Особенности реализации:
- 100% покрытие Tmin/Tmax в базе данных (не нужно обрабатывать NULL)
- Ограничение максимальных температур разумными пределами
- Обработка float precision для смежных интервалов
- Сохранение наиболее надежных данных при слиянии

Интеграция:
- Используется TemperatureFilterStage для фильтрации
- Интегрируется с PhaseBasedTemperatureStage
- Поддерживает FilterPipeline для конвейерной обработки
- Совместим с DatabaseRecord моделью

Используется в:
- TemperatureFilterStage для проверки температурного покрытия
- PhaseBasedTemperatureStage для сложной фильтрации
- CompoundSearcher для валидации результатов
- Тестировании температурных алгоритмов
"""
