...
class TemperatureFilterStage(FilterStage):
    ...
    def filter(
        ...
        self.last_stats = {
            ...
            "coverage_percentage": (len(filtered) / len(records) * 100)
            if records
            else 0,
...
class PhaseSelectionStage(FilterStage):
    ...
    def _calculate_phase_score(
        self, record: DatabaseRecord, expected_phase: Optional[str]
    ) -> float:
    ...
    def _has_adequate_thermodynamic_data(self, record: DatabaseRecord) -> bool:
        ...
        has_basic_data = (
            record.h298 is not None
            and record.s298 is not None
            and record.f1 is not None
            and record.f2 is not None
            and record.f3 is not None
        )
...
class TemperatureCoverageStage(FilterStage):
    ...
    def filter(
        ...
        """
        start_time = time.time()
        coverage_status = self.temperature_resolver.check_coverage(
    ...
    def _filter_by_coverage(
        self, records: List[DatabaseRecord], temperature_range: tuple
    ) -> List[DatabaseRecord]:
