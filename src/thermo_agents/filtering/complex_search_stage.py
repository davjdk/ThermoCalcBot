...
Специализированная стадия фильтрации для комплексного поиска химических формул.
Реализует многоуровневый алгоритм поиска на основе анализа базы данных с 316K записями.
Обеспечивает нахождение соединений, которые требуют расширенных паттернов поиска.

Основной класс:
- ComplexFormulaSearchStage: Стадия комплексного поиска формул

Стратегии поиска:
1. **Exact search**: TRIM(Formula) = 'H2O'
   - Точное совпадение формулы
   - Максимальная релевантность

2. **Phase search**: Formula LIKE 'H2O(%'
   - Поиск с указанием фазы в скобках
   - Для соединений с фазовыми спецификациями

3. **Prefix search**: Formula LIKE 'HCl%'
   - Префиксный поиск для сложных соединений
   - Важно для HCl, CO2, NH3, CH4

4. **Contains search**: Formula LIKE '%H2O%'
   - Общий поиск вхождения формулы
   - Самый низкий приоритет

Анализ формул:
- Простые молекулы, требующие расширенного поиска
- Сложные соединения с ионами и комплексами
- Соединения с полиморфными модификациями
- Гидраты и солевые системы

Особые случаи из анализа БД:
- HCl: 0 точных совпадений, 153 с префиксом
- CO2: 0 точных совпадений, 1428 с префиксом
- NH3: 1 точное совпадение, 1710 с префиксом
- CH4: 0 точных совпадений, 1352 с префиксом

Методы определения стратегии:
- _determine_search_method(): Выбор оптимальной стратегии
- _check_formula_match(): Проверка типа совпадения
- _is_complex_compound(): Анализ сложности формулы
- _needs_extended_search(): Определение необходимости расширенного поиска

Статистика поиска:
- exact_matches: Точные совпадения
- phase_matches: Совпадения с фазовой спецификацией
- prefix_matches: Префиксные совпадения
- contains_matches: Вхождения формулы

Оптимизации производительности:
- Кэширование результатов анализа формул
- Раннее прекращение поиска при точном совпадении
- Приоритизация более релевантных совпадений
- Минимизация строковых операций

Интеграция с FilterPipeline:
- Работает как стадия в конвейере фильтрации
- Использует FilterContext для передачи параметров
- Возвращает детальную статистику совпадений
- Совместима с DatabaseRecord моделью

Применение:
- Поиск сложных химических соединений
- Обработка синонимов и модификаций формул
- Нахождение всех вариаций записи формул
- Поддержка международных обозначений

Интеграция:
- Используется FilterPipeline для сложных запросов
- Интегрируется с другими стадиями фильтрации
- Поддерживает CompoundSearcher для поиска
- Совместим с остальными компонентами системы

Используется в:
- FilterPipeline для комплексного поиска
- CompoundSearcher для сложных формул
- Системах обработки химических запросов
