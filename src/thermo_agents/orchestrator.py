...
Центральный координатор гибридной архитектуры v2.0, объединяющий LLM-компоненты
и детерминированные модули через прямые вызовы для оптимальной производительности.
Поддерживает два режима: оптимизированный (ThermoOrchestrator) и с маршрутизацией (Orchestrator).

Ключевые компоненты:
- ThermoOrchestrator: Оптимизированный оркестратор с прямыми вызовами
- Orchestrator: Оркестратор с маршрутизацией по типам запросов
- OrchestratorConfig: Конфигурация без зависимостей от AgentStorage
- OrchestratorRequest/Response: Модели запросов и ответов

Основные методы ThermoOrchestrator:
- process_query(user_query): Обработка запроса с прямыми вызовами
- _search_and_filter_compound(): Поиск и фильтрация одного вещества
- _build_filter_statistics(): Преобразование FilterResult в статистику
- _format_response(): Форматирование финального ответа
- process_request(): Обработка для обратной совместимости
- shutdown(): Оптимизированное завершение работы

Основные методы Orchestrator:
- process_query(user_query): Обработка с маршрутизацией по типу
- _process_compound_data(): Обработка запросов данных веществ
- _process_reaction_calculation(): Обработка расчётов реакций
- shutdown(): Завершение работы

Поток выполнения ThermoOrchestrator:
1. Извлечение параметров через ThermodynamicAgent (LLM)
2. Поиск для каждого вещества (детерминированный)
3. Фильтрация для каждого вещества (детерминированный)
4. Агрегация результатов через ReactionAggregator
5. Форматирование через TableFormatter и StatisticsFormatter
...
Оптимизации:
- Убраны зависимости от AgentStorage
- Прямые вызовы вместо message passing
- Детерминированная логика для максимальной производительности
- Поддержка Unicode символов с fallback для Windows

Форматирование ответов:
- Использует emoji/symbols для визуализации
- Структурированный вывод с таблицами и статистикой
- Обработка предупреждений и ошибок
- Рекомендации для улучшения результатов

Интеграция:
- ThermodynamicAgent для извлечения параметров
- CompoundSearcher для поиска веществ
- FilterPipeline для фильтрации
- ReactionAggregator для агрегации
- TableFormatter/StatisticsFormatter для вывода
- SessionLogger для логирования

Используется в:
- ThermoSystem v2.0 как основной оркестратор
- CLI приложении main.py
- API endpoints для обработки запросов
"""
