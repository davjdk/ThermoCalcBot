...
class SearchStatistics(BaseModel):
    ...
    temperature_coverage: Optional[float] = Field(None, description="Temperature coverage fraction")
    avg_reliability: Optional[float] = Field(None, description="Average reliability class")

    # Temperature statistics
    min_temperature: Optional[float] = Field(None, description="Minimum temperature in records")
    max_temperature: Optional[float] = Field(None, description="Maximum temperature in records")
    avg_temperature_range: Optional[float] = Field(None, description="Average temperature range")

    # Phase distribution
    phase_distribution: Dict[str, int] = Field(default_factory=dict, description="Count of records per phase")

    # Reliability distribution
    reliability_distribution: Dict[int, int] = Field(default_factory=dict, description="Count of records per reliability class")
...
class CompoundSearchResult(BaseModel):
    ...
    records_found: List[DatabaseRecord] = Field(default_factory=list, description="Database records matching the search")
    search_parameters: Optional[Dict[str, Any]] = Field(None, description="Parameters used in search")

    # Results analysis
    coverage_status: CoverageStatus = Field(CoverageStatus.UNKNOWN, description="Coverage status")
    filter_statistics: Optional[SearchStatistics] = Field(None, description="Search statistics")
    ...
    def get_temperature_range(self) -> Optional[TemperatureRange]:
        ...
        valid_temps = [
            (r.tmin, r.tmax) for r in self.records_found
...
class SearchStrategy(BaseModel):
    ...
    estimated_difficulty: str = Field("medium", description="Estimated search difficulty")
...
class FilterOperation(BaseModel):
    ...
    filter_criteria: Optional[Dict[str, Any]] = Field(None, description="Filter criteria used")
    execution_time_ms: Optional[float] = Field(None, description="Filter execution time")
...
class SearchPipeline(BaseModel):
    ...
    operations: List[FilterOperation] = Field(default_factory=list, description="Filter operations performed")
    total_time_ms: Optional[float] = Field(None, description="Total pipeline execution time")
