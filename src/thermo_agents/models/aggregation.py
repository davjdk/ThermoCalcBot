...
class FilterStatistics(BaseModel):
    ...
    stage_3_description: str = Field(
        ...,
        description="Описание стадии 3 (выбор фазы)"
    )
    ...
    is_found: bool = Field(
        ...,
        description="Было ли вещество найдено в результате"
    )
    failure_stage: Optional[int] = Field(
        default=None,
        description="Стадия на которой произошёл провал (если применимо)"
    )
    failure_reason: Optional[str] = Field(
        default=None,
        description="Причина провала (если применимо)"
    )
...
class AggregatedReactionData(BaseModel):
    ...
    missing_compounds: List[str] = Field(
        ...
        description="Список ненайденных веществ"
    )
    found_compounds: List[str] = Field(
        default_factory=list,
        description="Список найденных веществ"
    )
    ...
    warnings: List[str] = Field(
        default_factory=list,
        description="Предупреждения о возможных проблемах"
    )
    recommendations: List[str] = Field(
        default_factory=list,
        description="Рекомендации пользователю"
    )
    ...
    def get_completeness_percentage(self) -> float:
        ...
        if self.get_total_compounds_count() == 0:
            return 0.0
        return (self.get_found_compounds_count() / self.get_total_compounds_count()) * 100