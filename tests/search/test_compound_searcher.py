...
class TestCompoundSearcher:
    ...
    def test_search_compound_with_temperature_range(self):
        ...
        result = self.searcher.search_compound("H2O", temperature_range=(300.0, 400.0))
    ...
    def test_search_compound_with_all_parameters(self):
        ...
        result = self.searcher.search_compound(
            "H2O", temperature_range=(300.0, 350.0), phase="l", limit=5
        )
    ...
    def test_search_compound_warnings_generation(self):
        ...
        reliability_warnings = [
            w for w in result.warnings if "low reliability" in w.lower()
        ]
        ...
        thermo_warnings = [w for w in result.warnings if "thermodynamic" in w.lower()]
    ...
    def test_search_compound_with_pipeline(self):
        ...
        assert isinstance(
            pipeline, Mock
        )  # Would be actual SearchPipeline in real implementation
    ...
    def test_determine_coverage_status_partial(self):
        ...
        records = [DatabaseRecord(id=1, formula="H2O", tmin=273.15, tmax=373.15)]
    ...
    def test_record_in_temperature_range(self):
        ...
        assert (
            self.searcher._record_in_temperature_range(record, temperature_range)
            is True
        )

        # Non-overlapping range
        assert (
            self.searcher._record_in_temperature_range(record, (400.0, 500.0)) is False
        )

        # Missing temperature data
        incomplete_record = {"Tmin": 273.15}
        assert (
            self.searcher._record_in_temperature_range(
                incomplete_record, temperature_range
            )
            is False
        )
    ...
    def test_search_compound_edge_cases(self):
        ...
        result = self.searcher.search_compound(
            "H2O", temperature_range=(5000.0, 6000.0)
        )
